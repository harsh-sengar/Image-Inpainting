...
os.makedirs("images_DPT_U_384_p6", exist_ok=True)
torch.manual_seed(0)
...
parser.add_argument("-m", "--model_weights", default="saved_model/DPT_gen_U_384_p1_5.pt", help="path to model weights")
...
parser.add_argument('--steps_2', type=int, default=20000)
...
writer = SummaryWriter('runs/DPT_U_experiment_new_p6')
...
def train(save_model=True):
    ...
    dis_opt = torch.optim.SGD((discriminator.parameters()), lr = lr/10)
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[30000, 150000, 400000], gamma=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                writer.add_scalar('Generator training DPT p6 loss p1',
                            g_loss.item(),
                            iteration)
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_p6/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p6/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p6/p1_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        while pbar.n < args.steps_2:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                writer.add_scalar('Generator training DPT p6 loss p2',
                            g_loss.item() ,
                            iteration)
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_p6/p2_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p6/p2_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p6/p2_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        while pbar.n < args.steps_3:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                writer.add_scalar('discriminator training DPT p6 loss p3',
                            loss.item(),
                            iteration)
        ...
        while pbar.n < args.steps_4:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                writer.add_scalar('Discriminator training DPT p6 loss p4',
                            d_loss.item() ,
                            iteration)

                writer.add_scalar('Generator training DPT p6 loss p4',
                            g_loss.item() ,
                            iteration)
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_p6/p4_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p6/p4_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p6/p4_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        plt.legend()
        # plt.show()
        plt.savefig('plot/plot_DPT_p6_epochs_new_%d.png'%( epoch)) 

        if save_model:
            gen_name = os.path.join("saved_model/", 'DPT_gen_U_384_new_p6_%d.pt'%epoch)
            dis_name = os.path.join("saved_model/", 'DPT_dis_U_384_new_p6_%d.pt'%epoch)
            opt_name = os.path.join("saved_model/", 'optimizer_DPT_U_p6_new_%d.pt'%epoch)
