...
os.makedirs("images_DPT_disc", exist_ok=True)
torch.manual_seed(0)
cuda = True if torch.cuda.is_available() else False
os.environ["CUDA_VISIBLE_DEVICES"]="1"

writer = SummaryWriter('runs/DPT_GAN_experiment_disc')
...
parser.add_argument('--steps_1', type=int, default=9000)
...
def train(save_model=True):
        ...
        while pbar.n < args.steps_2:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    if cur_step > 0:
                        print(f"Epoch {epoch}: Step {cur_step}: Discriminator Loss: {mean_discriminator_loss/ display_step}")
                    else:
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if(cur_step < 10000):
                    ...
                    loss_cn_1 = mse_loss(gen_output * mask, imgs * mask)
                # loss_cn_1 = pixelwise_loss(imgs*mask, gen_output*mask)
                ...
                writer.add_scalar('generator training loss p3',
                            mean_generator_loss,
                            epoch * len(train_dataloader) + i)

                # # create grid of images
                # img_grid = torchvision.utils.make_grid(gen_output)

                # # show images
                # matplotlib_imshow(img_grid, one_channel=False)

                # # write to tensorboard
                # writer.add_image('generated_images_p1', img_grid)
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_disc/p3_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_disc/p3_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_disc/p3_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
    ...
    if save_model:
        gen_name = os.path.join("saved_model/", 'DPT_GAN_gen%d.pt'%n_epochs)
        dis_name = os.path.join("saved_model/", 'DPT_GAN_dis.pt')
        opt_name = os.path.join("saved_model/", 'optimizer_DPT_GAN_%d.pt'%n_epochs)
