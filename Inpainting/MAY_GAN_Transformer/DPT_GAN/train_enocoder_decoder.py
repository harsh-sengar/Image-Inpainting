...
from dpt.models import Generator, DPTInpainting, ContextDiscriminator
from dpt.transforms import NormalizeImage

#from util.misc import visualize_attention
os.makedirs("saved_model", exist_ok=True)
os.makedirs("images_DPT_4", exist_ok=True)
torch.manual_seed(0)
...
writer = SummaryWriter('runs/DPT_GAN_experiment_new_1')
...
def train(save_model=True):
    ...
    dis_opt = torch.optim.SGD((discriminator.parameters()), lr = lr/100)
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[10000, 20000, 50000, 100000], gamma=0.1)
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_4/p1_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_4/p1_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_4/p1_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_4/p2_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_4/p2_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_4/p2_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_4/p4_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_4/p4_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_4/p4_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
    ...
    if save_model:
        gen_name = os.path.join("saved_model/", 'DPT_GAN_gen_4_%d.pt'%n_epochs)
        dis_name = os.path.join("saved_model/", 'DPT_GAN_dis_4_.pt')
        opt_name = os.path.join("saved_model/", 'optimizer_DPT_GAN_4_%d.pt'%n_epochs)
