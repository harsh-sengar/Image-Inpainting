...
os.makedirs("images_DPT_U_384_p5", exist_ok=True)
torch.manual_seed(0)
...
writer = SummaryWriter('runs/DPT_U_experiment_new_p5')
    ...
    for epoch in range(n_epochs):
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_p5/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p5/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p5/p1_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_p5/p2_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p5/p2_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p5/p2_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        while pbar.n < args.steps_4:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                loss_cn_1 = pixelwise_loss(gen_output, imgs)
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_p5/p4_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_384_p5/p4_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_384_p5/p4_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        plt.legend()
        # plt.show()
        plt.savefig('plot/plot_DPT_p5_epochs_new_%d.png'%( epoch)) 

        if save_model:
            gen_name = os.path.join("saved_model/", 'DPT_gen_U_384_p5_new_%d.pt'%epoch)
            dis_name = os.path.join("saved_model/", 'DPT_dis_U_384_p5_new_%d.pt'%epoch)
            opt_name = os.path.join("saved_model/", 'optimizer_DPT_U_p5_new_%d.pt'%epoch)
