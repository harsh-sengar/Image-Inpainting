...
os.makedirs("images_DPT1", exist_ok=True)
torch.manual_seed(0)
...
def train(save_model=True):
    ...
    G_losses = []
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[1000, 20000, 50000, 100000], gamma=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        for i, (imgs, masked_imgs, masked_parts) in enumerate(dataloader):  
            ...
            if cur_step % display_step == 0:
                ...
                save_image(imgs.data, "images_DPT1/%d.png" % cur_step, nrow=1, normalize=True)
                save_image(masked_imgs.data, "images_DPT1/%d_mi.png" % cur_step, nrow=1, normalize=True)
                save_image((gen_parts.data+1)/2, "images_DPT1/%d_gp.png" % cur_step, nrow=1, normalize=False)
                mean_generator_loss = 0
    ...
    plt.legend()
    # plt.show()
    plt.savefig('plot/plot_DPT_batch_%d_epoch_LR_1_%d.png'%(batch_size, n_epochs)) 
    if save_model:
        gen_name = os.path.join("saved_model/", 'DPT_epoch_LR_1_%d.pt'%n_epochs)
        opt_name = os.path.join("saved_model/", 'optimizer_DPT_epoch_LR_1_%d.pt'%n_epochs)
