...
os.makedirs("images_inpaint_encoder_disc", exist_ok=True)
os.makedirs("images_inpaint_encoder_disc_test", exist_ok=True)
torch.manual_seed(0)
...
writer = SummaryWriter('runs/DPT_encoder_disc_experiment')
    ...
    for epoch in range(n_epochs):
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    for i in range(len(imgs)):
                        save_image(x_mask[i].data, "images_inpaint_encoder_disc/p1_%d_%d_%d_masked.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image(imgs[i].data, "images_inpaint_encoder_disc/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image((gen_output[i].data+1)/2, "images_inpaint_encoder_disc/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if cur_step % display_step == 0:
                    ...
                    for i in range(len(imgs)):
                        save_image(x_mask[i].data, "images_inpaint_encoder_disc/p4_%d_%d_%d_masked.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image(imgs[i].data, "images_inpaint_encoder_disc/p4_%d_%d_%d_.png"  % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image((gen_output[i].data+1)/2, "images_inpaint_encoder_disc/p4_%d_%d_%d_gen_output.png"  % (i, cur_step, epoch), nrow=1, normalize=False)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
        ...
        if save_model:
            ...
            for i, (samples, _, _) in enumerate(test_dataloader):  
                ...
                # Save sample
                save_image(samples.data, "images_inpaint_encoder_disc_test/t_%d_%d_sample.png" % (i, epoch), nrow=1, normalize=True)
                save_image((gen_mask+1)/2, "images_inpaint_encoder_disc_test/t_%d_%d.png" % (i, epoch), nrow=1, normalize=False)
