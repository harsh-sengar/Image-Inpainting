...
os.makedirs("images_DPT_GAN1", exist_ok=True)
torch.manual_seed(0)
...
def train(save_model=True):
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[5000, 20000, 750000, 150000], gamma=0.1)
    ...
    for epoch in range(n_epochs):
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    for i in range(len(imgs)):
                        save_image(imgs[i].data, "images_DPT_GAN1/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image(gen_output[i].data, "images_DPT_GAN1/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if cur_step % display_step == 0:
                    ...
                    for i in range(len(imgs)):
                        save_image(imgs[i].data, "images_DPT_GAN1/p4_%d_%d_%d_.png"  % (i, cur_step, epoch), nrow=1, normalize=True)
                        save_image(gen_output[i].data, "images_DPT_GAN1/p4_%d_%d_%d_gen_output.png"  % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
        ...
        if save_model:
            gen_name = os.path.join("saved_model/", 'DPT_GAN1_Gen_epoch_%d_.pt'%(epoch))
            dis_name = os.path.join("saved_model/", 'DPT_GAN1_disc_epoch_%d_.pt'%(epoch))
            opt_name = os.path.join("saved_model/", 'optimizer_DPT1_GAN_%d_.pt'%(epoch))
