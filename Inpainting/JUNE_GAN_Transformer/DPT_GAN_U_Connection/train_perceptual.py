...
os.makedirs("images_DPT_U_Connection_384_2", exist_ok=True)
os.makedirs("test_DPT_U_Connection_384_2", exist_ok=True)
os.makedirs("val_DPT_U_Connection_384_2", exist_ok=True)
torch.manual_seed(0)
...
parser.add_argument("-m", "--model_weights", default="saved_model/DPT_gen_conn_U_384_25.pt", help="path to model weights")
...
writer = SummaryWriter('runs/DPT_U_experiment2')
...
def save_sample(generator, val_dataloader, epoch, mode):
    j = 0
    for i, (imgs, _, _) in enumerate(val_dataloader):  
        ...
        if(mode == "val"):
            save_image(imgs.data, "val_DPT_U_Connection_384_2/val_%d_%d.png" % (j, epoch), nrow=1, normalize=True)
            save_image((gen_output.data+1)/2, "val_DPT_U_Connection_384_2/val_%d_%d_gen_output.png" % (j, epoch), nrow=1, normalize=False)
            save_image(x_mask.data, "val_DPT_U_Connection_384_2/val_%d_%d_mask.png" % (j, epoch), nrow=1, normalize=True)
        if(mode == "test"):
            save_image(imgs.data, "test_DPT_U_Connection_384_2/test_%d_%d.png" % (j, epoch), nrow=1, normalize=True)
            save_image((gen_output.data+1)/2, "test_DPT_U_Connection_384_2/test_%d_%d_gen_output.png" % (j, epoch), nrow=1, normalize=False)
            save_image(x_mask.data, "test_DPT_U_Connection_384_2/test_%d_%d_mask.png" % (j, epoch), nrow=1, normalize=True)
        j = j+1
...
def train(save_model=True):
    ...
    # print(generator)
#     summary(DPTInpainting(
#         path=args.model_weights,
#         backbone="vitb_rn50_384",
#         non_negative=True,
#         enable_attention_hooks=False,
#     ).to(device)
# , input_size=(3, 384, 384),verbose = 0,


#     )
    # exit(0)

    gen_opt =  torch.optim.SGD((generator.unet.parameters()), lr = lr)
    dis_opt = torch.optim.SGD((discriminator.parameters()), lr = lr/25)
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[40000, 150000, 400000], gamma=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                p_loss = perceptual_loss((gen_output+1)/2, (imgs+1)/2).sum()

                g_loss = g_loss + p_loss * alpha
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_Connection_384_2/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384_2/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384_2/p1_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        #  Training Phase 2
        # ---------------------
        # Training Completion Mask Network (Transformer)
        ...
        while pbar.n < args.steps_2:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                g_loss = pixelwise_loss(gen_output * mask, imgs * mask)*16
                ...
                writer.add_scalar('Generator training loss p2',
                            g_loss.item() ,
                            iteration)

                if cur_step % display_step == 0:
                    if cur_step > 0:
                        print(f"Epoch {epoch}: Step {cur_step}: Generator DPT loss mask: {mean_generator_loss/ display_step}")
                    else:
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_Connection_384_2/p2_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384_2/p2_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384_2/p2_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        while pbar.n < args.steps_3:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                # optimize
                dis_opt.step()
                pbar.set_description('phase 3 | train loss: %.5f' % loss.cpu())
                pbar.update()

                D_losses.append(loss.item())
                mean_discriminator_loss += loss.item() 
                writer.add_scalar('discriminator training loss p3',
                            loss.item(),
                            iteration)
        ...
        G_losses = []
        D_losses = []
        pbar = tqdm(total=args.steps_4)
        while pbar.n < args.steps_4:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                gen_opt.zero_grad()
                dis_opt.zero_grad()
                ...
                loss_cn_1 = pixelwise_loss(gen_output * mask, imgs * mask)*16
                ...
                pbar.set_description(
                    'phase 4 | train loss (cd): %.5f (cn): %.5f' % (
                        d_loss.cpu(),
                        g_loss.cpu()))
                pbar.update()
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        print(f"Epoch {epoch}: Step {cur_step}: Generator DPT loss: {mean_generator_loss/ display_step}, Discriminator Loss: {mean_discriminator_loss/ display_step}")
                    else:
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_Connection_384_2/p4_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384_2/p4_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384_2/p4_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
                iteration += 1
                if pbar.n >= args.steps_4:
                    break
        pbar.close()
        ...
        plt.legend()
        # plt.show()
        plt.savefig('plot/plot_DPT_epochs_%d.png'%( epoch)) 

        if save_model:
            gen_name = os.path.join("saved_model/", 'DPT_gen_conn_U_384_2_%d.pt'%epoch)
            dis_name = os.path.join("saved_model/", 'DPT_dis__conn_U_384_2_%d.pt'%epoch)
            opt_name = os.path.join("saved_model/", 'optimizer_DPT_conn_U_2_%d.pt'%epoch)
