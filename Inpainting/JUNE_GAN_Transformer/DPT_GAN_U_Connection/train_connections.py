...
os.makedirs("images_DPT_U_Connection_384", exist_ok=True)
os.makedirs("test_DPT_U_Connection_384", exist_ok=True)
os.makedirs("val_DPT_U_Connection_384", exist_ok=True)
torch.manual_seed(0)
...
def save_sample(generator, val_dataloader, epoch, mode):
    ...
    for i, (imgs, _, _) in enumerate(val_dataloader):  
        ...
        if(mode == "val"):
            save_image(imgs.data, "val_DPT_U_Connection_384/val_%d_%d.png" % (j, epoch), nrow=1, normalize=True)
            save_image((gen_output.data+1)/2, "val_DPT_U_Connection_384/val_%d_%d_gen_output.png" % (j, epoch), nrow=1, normalize=False)
            save_image(x_mask.data, "val_DPT_U_Connection_384/val_%d_%d_mask.png" % (j, epoch), nrow=1, normalize=True)
        if(mode == "test"):
            save_image(imgs.data, "test_DPT_U_Connection_384/test_%d_%d.png" % (j, epoch), nrow=1, normalize=True)
            save_image((gen_output.data+1)/2, "test_DPT_U_Connection_384/test_%d_%d_gen_output.png" % (j, epoch), nrow=1, normalize=False)
            save_image(x_mask.data, "test_DPT_U_Connection_384/test_%d_%d_mask.png" % (j, epoch), nrow=1, normalize=True)
        j = j+1
    ...
    for epoch in range(n_epochs):
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                # p_loss = perceptual_loss(gen_output, imgs).sum()

                # loss = g_loss + p_loss * alpha
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_Connection_384/p1_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384/p1_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384/p1_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_Connection_384/p2_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384/p2_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384/p2_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
                iteration += 1
             ...
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_Connection_384/p4_%d_%d_%d.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    save_image((gen_output.data+1)/2, "images_DPT_U_Connection_384/p4_%d_%d_%d_gen_output.png" % (i, cur_step, epoch), nrow=1, normalize=False)
                    save_image(x_mask.data, "images_DPT_U_Connection_384/p4_%d_%d_%d_mask.png" % (i, cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
                iteration += 1
        ...
        if save_model:
            gen_name = os.path.join("saved_model/", 'DPT_gen_U_384_%d.pt'%epoch)
            dis_name = os.path.join("saved_model/", 'DPT_dis_U_384_%d.pt'%epoch)
            opt_name = os.path.join("saved_model/", 'optimizer_DPT_U_%d.pt'%epoch)
