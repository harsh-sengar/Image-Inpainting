...
os.makedirs("images_DPT_U_384_2", exist_ok=True)
torch.manual_seed(0)
...
writer = SummaryWriter('runs/DPT_U_experiment_new')
...
def train(save_model=True):
    ...
    scheduler_g = torch.optim.lr_scheduler.MultiStepLR(gen_opt, milestones=[20000, 150000, 200000], gamma=0.1)
    ...
    for epoch in range(n_epochs):
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_2/p1_%d_%d.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_2/p1_%d_%d_gen_output.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_2/p1_%d_%d_mask.png" % (cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                g_loss = mse_loss(gen_output * mask, imgs * mask)*16
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_2/p2_%d_%d.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_2/p2_%d_%d_gen_output.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_2/p2_%d_%d_mask.png" % (cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                loss_cn_1 = mse_loss(gen_output * mask, imgs * mask)*16
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_2/p4_%d_%d.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_2/p4_%d_%d_gen_output.png" % (cur_step, epoch), nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_2/p4_%d_%d_mask.png" % (cur_step, epoch), nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
    ...
    plt.legend()
    # plt.show()
    plt.savefig('plot/plot_DPT_GAN_%d_epochs_%d.png'%(batch_size, n_epochs)) 

    if save_model:
        gen_name = os.path.join("saved_model/", 'DPT_gen_U_384_2_%d.pt'%n_epochs)
        dis_name = os.path.join("saved_model/", 'DPT_dis_U_384_2_%d.pt'%n_epochs)
        opt_name = os.path.join("saved_model/", 'optimizer_DPT_U_2_%d.pt'%n_epochs)
