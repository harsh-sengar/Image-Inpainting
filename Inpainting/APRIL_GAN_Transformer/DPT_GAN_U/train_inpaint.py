...
os.makedirs("images_DPT_U_384_p1", exist_ok=True)
torch.manual_seed(0)
...
writer = SummaryWriter('runs/DPT_U_experiment_new_p1')
...
def train(save_model=True):
    ...
    dis_opt = torch.optim.SGD((discriminator.parameters()), lr = lr/20)
        ...
        while pbar.n < args.steps_1:
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_p1/p1_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_p1/p1_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_p1/p1_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
            ...
            for i, (imgs, _, _) in enumerate(train_dataloader):  
                ...
                if cur_step % display_step == 0:
                    if cur_step > 0:
                        ...
                        print("Pretrained initial state")
                    save_image(imgs.data, "images_DPT_U_384_p1/p2_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_p1/p2_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_p1/p2_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                cur_step += 1
        ...
        while pbar.n < args.steps_3:
             for i, (imgs, _, _) in enumerate(train_dataloader): 
                ...
                if cur_step % display_step == 0:
                    ...
                    save_image(imgs.data, "images_DPT_U_384_p1/p4_%d.png" % cur_step, nrow=1, normalize=True)
                    save_image(gen_output.data, "images_DPT_U_384_p1/p4_%d_gen_output.png" % cur_step, nrow=1, normalize=True)
                    save_image(x_mask.data, "images_DPT_U_384_p1/p4_%d_mask.png" % cur_step, nrow=1, normalize=True)
                    mean_generator_loss = 0
                    mean_discriminator_loss = 0
                cur_step += 1
    ...
    if save_model:
        gen_name = os.path.join("saved_model/", 'DPT_gen_U_384_p_%d.pt'%n_epochs)
        dis_name = os.path.join("saved_model/", 'DPT_dis_U_384_p_%d.pt'%n_epochs)
        opt_name = os.path.join("saved_model/", 'optimizer_DPT_U_p_%d.pt'%n_epochs)
